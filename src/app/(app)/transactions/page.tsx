...
const INITIAL_MOCK: Transaction[] = [
    { id: '1', name: 'Salário', desc: 'Empresa XYZ Ltda', amount: 12500, type: 'in', category: 'Salário', categoryIcon: '💰', date: new Date().toISOString().split('T')[0], account: 'Nubank' },
    { id: '2', name: 'Aluguel', desc: 'Apartamento Campinas', amount: 2800, type: 'out', category: 'Moradia', categoryIcon: '🏠', date: new Date().toISOString().split('T')[0], account: 'Itaú' },
...
export default function TransactionsPage() {
    const [transactions, setTransactions] = useState<Transaction[]>(INITIAL_MOCK)
    ...
    const handleSave = () => {
        ...
        const newTx: Transaction = {
            ...
            name: desc.split(' ')[0] || 'Novo',
            desc: desc,
            amount: parseFloat(amount),
            type: type,
            category: type === 'in' ? 'Entrada' : 'Geral',
            categoryIcon: type === 'in' ? '📈' : '🏷️',
            date: date,
        ...
        <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 16 }}>
                ...
                <div style={{ display: 'flex', gap: 12 }}>
                    <button onClick={() => toast('Download iniciado', { style: { background: C.card, color: C.text, border: `1px solid ${C.border}` } })} style={btnOutlineStyle}>
                    ...
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                        ...
                        <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }}
                            ...
                            <div style={{ marginBottom: 16 }}>
                                ...
                                <input type="number" value={amount} onChange={e => setAmount(e.target.value)} placeholder="0,00" style={{ ...inputStyle, fontSize: 24, fontWeight: 700 }} />
                            </div>
                            ...
                            <div style={{ display: 'flex', gap: 12, marginTop: 24 }}>
                                ...
                                <button onClick={handleSave} style={{ ...btnGoldStyle, flex: 1, padding: '12px 0' }}>Salvar</button>
                            </div>
                        </motion.div>
                    </motion.div>
                )}
